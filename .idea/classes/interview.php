...
/*
 * 39. Есть массив a = array(тут много элементов). Проходим по массиву циклом
...
/*
 * 40. Вывести максимальное значение элемента массива array(1,2,3,4,10,100,3,
 * 4987,6,7,8,9).
 */

$arr = array(1, 2, 3, 4, 10, 100, 3,
    4987, 6, 7, 8, 9);
echo max($arr);
echo "\r\n";
echo "***\r\n";
...
/*
 * 41. Напиши программу-цензор, которая бы заменяла вводимые пользователем
 ...
 * __set_state
 * __clone
 * __construct и __destruct – самые популярные методы, которые реализуют базовые понятия
 *  объектно-ориентированного программирования: конструктор и деструктор;
 ...
 *  __sleep и __wakeup – методы, которые вызываются только из функций serialize и unserialize
 * соответственно. Метод __sleep будет вызван сразу при применении к объекту функции serialize, а
...
/* 43. Назови по памяти любые функции для работы со строками, массивами и
* файлами.
 *  Например, такие:
 * Строки:
